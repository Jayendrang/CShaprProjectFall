...
PRODUCT.PRICE_PER_UNIT,PRODUCT.MINIMUM_COUNT,MANUFACTURER.MANUFACTURER_ID, MANUFACTURER.MANUFACTURER_NAME,MANUFACTURER_DETAIL FROM PRODUCT INNER JOIN MANUFACTURER ON PRODUCT.MANUFACTURER_ID = MANUFACTURER.MANUFACTURER_ID WHERE MANUFACTURER.MANUFACTURER_ID='MAN002' ORDER BY MANUFACTURER.MANUFACTURER_NAME;
 ...
 select top 1 DISCOUNT_id from discount order by DISCOUNT_ID desc; 
 select top 1 manufacturer_id from MANUFACTURER order by MANUFACTURER_id desc; 
 select top 1 order_id from SALES_ORDER order by ORDER_id desc;
 select top 1 RETURN_ORDER_ID from RETURN_ORDER order by ORDER_ID desc;
 select top 1 USER_ID from USER_TABLE order by USER_ID desc;
 select top 1 vendor_id from VENDOR order by VENDOR_ID desc;
